...
import { Palette, Sun, Settings } from 'lucide-react';
import './index.css';
...
const Popup = () => {
  ...
  useEffect(() => {
    chrome.storage.local.get(['readerEnabled', 'readerBg', 'readerText', 'readerLink', 'readerBrightness'], (res) => {
    ...
    <div className="p-4 bg-base-100 min-h-[400px] border border-base-300 rounded-lg shadow-xl overflow-hidden" data-theme="dark">
      <div className="flex items-center justify-between mb-6 border-b border-base-300 pb-3">
        <h1 className="text-lg font-bold flex items-center gap-2">
          <Settings size={20} className="text-primary" /> MarkBrew Reader
        </h1>
      </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="form-control">
            <label className="label">
              <span className="label-text text-[10px]">Background</span>
            </label>
            <input
              type="color"
          ...
          <div className="form-control">
            <label className="label">
              <span className="label-text text-[10px]">Text</span>
            </label>
            <input
              type="color"
              className="w-full h-8 rounded cursor-pointer bg-transparent border-none"
          ...
          <div className="form-control">
            <label className="label">
              <span className="label-text text-[10px]">Link</span>
            </label>
            <input
              type="color"
              className="w-full h-8 rounded cursor-pointer bg-transparent border-none"
        ...
        <div className="form-control">
          <label className="label">
            <span className="label-text-alt font-medium flex items-center gap-1">
              <Sun size={14} /> Brightness ({Math.round(brightness * 100)}%)
            </span>
          </label>
          <input
      ...
      <div className="mt-8 pt-4 border-t border-base-300 text-[10px] text-center opacity-50">
        Changes are applied instantly to all tabs.
      </div>
    </div>
  );
};
