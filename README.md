...
**MarkBrew** は、ブラウザの「新しいタブ」を生産性の高いダッシュボードと高度なブックマーク管理ハブに変える強力な Chrome 拡張機能です。
...
- **天気予報**: 位置情報を使用したリアルタイムな現地の天気更新（手動更新対応）。
- **RSS フィード**: お気に入りのニュースソースをチェック。
- **システムモニター**: CPU、メモリ、ストレージ、ディスプレイの使用状況をリアルタイムで監視。
- **電卓ウィジェット**: プログラマー向け機能（DEC / HEX / BIN 変換）を備えた安全な電卓。
- **ウィジェットの並び替え**: ドラッグ＆ドロップで自由に配置を変更可能。
- **レイアウトの最適化**: 3カラム制限と正方形ウィジェットで整然としたデザイン。

### 🔖 高度なブックマーク管理
MarkBrew は単なるリスト表示を超え、4 つの異なるスタイルでブックマークを可視化・操作できます：
...
- **右クリックメニュー**: 選択したテキストを右クリックから直接「MarkBrew」に保存。
- **フォーマット維持**: 改行やインデントを正しく保持して保存（長文コードに対応した1カラム表示）。
